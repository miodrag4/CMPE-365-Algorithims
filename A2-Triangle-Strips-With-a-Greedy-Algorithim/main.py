...
def bestTri(triangles):
    # Starting value
    start = None
    # Upper bound  for replacements. Size slightly larger than necessary to be safe.
    best_upper_bound = 5
    upper_length_bound = 5
    # Triangle Id
    i = 0
    # Loops through all the elements of triangles[]
    while i != len(triangles):
        tri = triangles[i]
        # Checks if the next and previous possible element is in a strip already
        if tri.nextTri is None and tri.prevTri is None:
            # Find the number of adjacent triangles the current triangle has
            next_one = len(tri.adjTris)
            # Find the triangles adjacent-adjacent triangles only when both next and prev pointer is none
            # but, we noticed when the condition for the next tri and prev tri is set to "or" instead of "and"
            # the 200 strips went to 11 and the 10000 went to 524.
            temp = [adjs for adjs in tri.adjTris if adjs.nextTri is None and adjs.prevTri is None]
            tri_length = len(temp)
            # Condition to start at the triangle with the least adjacent triangles
            if next_one <= best_upper_bound and tri_length <= upper_length_bound:
                upper_length_bound = tri_length
                best_upper_bound = next_one
                start = tri
        # increment value to go through the list
        i += 1
    # Returns the new start value based on the conditions above
    return start


# Function to determine the next triangle
def NextTri(triangle):
    while True:
        # Next Triangle is the triangle with best option.
        triangle.nextTri = bestTri(triangle.adjTris)
        if triangle.nextTri is not None:
            # Double linked list is now the start of the list
            triangle.nextTri.prevTri = triangle
            # The start of the list points to the Next triangle
            triangle = triangle.nextTri
        else:
            triangle.prevTri = triangle
            return


def buildTristrips(triangles):
    # Number of strips counter
    count = 0
    # Triangle counter for the while loop
    tri = 0
    # Loop through entre list of triangles until they are all visited
    while tri != len(triangles):
        # Start at the best triangle in the list by calling the bestTri function, with the triangles list.
        start = bestTri(triangles)
        # Condition for when the list finishes, break and find the next triangle to start at.
