...
def buildHull(points):
    ...
    # Handling the base case for two points
    if len(points) == 2:
        ...
        return
    # Handling the base case for three points
    if len(points) == 3:
    ...
    # Split the points into two parts with half being allocated to the left and the other to the right
    left_points = points[:len(points) // 2]
    right_points = points[len(points) // 2:]
    # Recursively call BuildHull with the right and left points
    ...
    # Keep record of points moved past during the walk up and down processes
    old_points = set()
    ...
    # Given that the left and rights points have room to step up execute this while loop
    # This is the algorithm to in the week 6 lecture 3 notes
    while (
    ...
    # Saving reference points for walk down
    ...
    # Remove points that interfere with cw/ccw references
    old_points -= {topRight, topLeft, bottom_right, bottom_left}

    # Remove referenes for the points stored in
